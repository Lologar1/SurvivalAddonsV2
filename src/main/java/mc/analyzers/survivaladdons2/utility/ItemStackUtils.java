...
public class ItemStackUtils {
    ...
    public static void syncItem(ItemStack item){
        ...
        if(getCustomEnchantments(item).containsKey("rush")){
            AttributeModifier speed = new AttributeModifier(UUID.randomUUID(), "generic.movementSpeed", 0.004*getCustomEnchantments(item).get("rush"), AttributeModifier.Operation.ADD_NUMBER, equipmentSlot);
            meta.addAttributeModifier(Attribute.GENERIC_MOVEMENT_SPEED, speed);
        }else{
            AttributeModifier speed = new AttributeModifier(UUID.randomUUID(), "generic.movementSpeed", 0, AttributeModifier.Operation.ADD_NUMBER, equipmentSlot);
            meta.addAttributeModifier(Attribute.GENERIC_MOVEMENT_SPEED, speed);
        }
        ...
        if(PDCUtils.has(item, "enchantments") && !Objects.equals(PDCUtils.get(item, "enchantments"), "")){
            ...
            for(String name : customEnchantments.keySet()){
                ...
                if(item.getType().equals(Material.ENCHANTED_BOOK)){ //Thingy
                    ...
                    ArrayList<String> conflicts = (ArrayList<String>) Arrays.stream(getById(name).getConflicts()).collect(Collectors.toList());
                    ArrayList<Enchantment> vanillaConflicts = (ArrayList<Enchantment>) Arrays.stream(getById(name).getVanillaConflicts()).collect(Collectors.toList());
                    vanillaConflicts.forEach((enchantment -> conflicts.add(enchantment.getKey().getKey())));
                    if(!conflicts.isEmpty()){
                        lore.add(ChatColor.RED + "Conflicts :");
                        StringBuilder conflictString = new StringBuilder();
                        conflicts.stream().forEach(conflict -> conflictString.append(capFirst(conflict)).append(" "));
                        lore.add(ChatColor.GRAY + conflictString.toString());
                    }
                }
            }
        }
    ...
    public static void createCooldown(ItemStack item, String cooldownId, long spanMillis){
        PDCUtils.set(item, cooldownId, System.currentTimeMillis() + "/" + spanMillis);
    }
    public static boolean checkCooldown(ItemStack item, String cooldownId){
        boolean can = false;
        if(!PDCUtils.has(item, cooldownId)){
            return false;
        }
        String data = PDCUtils.get(item, cooldownId);
        long lastTime = Long.parseLong(data.split("/")[0]);
        long span = Long.parseLong(data.split("/")[1]);
        if(System.currentTimeMillis() - lastTime >= span){
            can = true;
        }
        return can;
    }
    public static long getRemainingCooldown(ItemStack item, String cooldownId){
        String data = PDCUtils.get(item, cooldownId);
        if(data == null){
            PDCUtils.set(item, cooldownId, "0/0");
        }
        long lastTime = Long.parseLong(data.split("/")[0]);
        long span = Long.parseLong(data.split("/")[1]);
        long remaining = span - (System.currentTimeMillis() - lastTime);
        if(remaining < 0){
            return 0;
        }
        return remaining;
